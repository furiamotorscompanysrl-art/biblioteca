...
def get_drive_service():
    ...
    ✅ PRIORIZA OAuth (usuario real) - TIENE CUOTA
    ✅ Fallback a Cuenta de Servicio - NO TIENE CUOTA
    """
    try:
        # ============================================
        ...
        if creds_json and token_json:
            try:
                ...
                logger.info("✅ Servicio Drive desde OAuth (usuario real) - TIENE CUOTA")
                return service
        ...
        if token_base64 and creds_base64:
            try:
                ...
                logger.info("✅ Servicio Drive desde base64")
                return service
            except Exception as e:
                logger.warning(f"⚠️ Error con base64: {e}")
        
        # ============================================
        # TERCERO: Cuenta de Servicio (SIN CUOTA)
        # ============================================
        try:
            ...
            if creds_json:
                ...
                service = build('drive', 'v3', credentials=credentials)
                logger.warning("⚠️ Servicio Drive desde Cuenta de Servicio - SIN CUOTA")
                return service
        except Exception as e:
...
def subir_pdf_a_drive(archivo_pdf, nombre_archivo=None, folder_path='Material_Biblioteca/Libros/PDFs'):
    ...
    try:
        ...
        logger.info(f"📤 Subiendo PDF: {nombre_limpio} ({len(contenido) / 1024:.1f} KB)")
        ...
        try:
            ...
            logger.warning(f"⚠️ No se pudo hacer público el PDF: {e}")
...
def subir_pdf_a_drive_from_bytes(contenido_bytes, nombre_archivo, folder_path='Material_Biblioteca/Libros/PDFs'):
    """Sube un PDF a Google Drive desde bytes."""
    try:
        service = get_drive_service()
        if not service:
            logger.error("❌ No se pudo obtener servicio de Drive")
...
def subir_imagen_a_drive_from_bytes(contenido_bytes, nombre_archivo, folder_path='Material_Biblioteca/Libros/Portadas'):
    """Sube una imagen a Google Drive desde bytes."""
    try:
        service = get_drive_service()
        if not service:
            logger.error("❌ No se pudo obtener servicio de Drive")
        ...
        try:
            ...
            logger.warning(f"⚠️ No se pudo hacer pública la imagen: {e}")
        
        # URL que funciona en <img>
...
def subir_imagen_a_drive(archivo_imagen, nombre_archivo=None, folder_path='Material_Biblioteca/Libros/Portadas'):
    ...
    try:
        ...
        logger.info(f"📤 Subiendo imagen: {nombre_limpio} ({len(contenido) / 1024:.1f} KB)")
        file = service.files().create(
            body=file_metadata,
            media_body=media,
            fields='id, webViewLink',
    ...
    def upload_thread():
        try:
            ...
            if drive_url:
                ...
                logger.info(f"✅ PDF subido: {drive_url} (Libro ID: {libro_id})")
            else:
    ...
    def upload_thread():
        try:
            ...
            if drive_url:
                ...
                logger.info(f"✅ Portada subida: {drive_url} (Libro ID: {libro_id})")
            else:
...
def subir_autorizacion_a_drive_async(archivo, nombre_archivo, libro_id, folder_path='Material_Biblioteca/Autorizaciones'):
    """Sube una autorización a Google Drive en segundo plano"""
    try:
        contenido_bytes = archivo.read()
        archivo.seek(0)
        nombre_original = archivo.name if hasattr(archivo, 'name') else f'{nombre_archivo}.pdf'
    except Exception as e:
        logger.error(f"❌ Error leyendo autorización antes de subir: {e}")
        return None
    ...
    def upload_thread():
        try:
            ...
            if drive_url:
                ...
                logger.info(f"✅ Autorización subida: {drive_url} (Libro ID: {libro_id})")
            else:
                logger.error(f"❌ Fallo subida de autorización para libro {libro_id}")
        except Exception as e:
            logger.error(f"❌ Error en subida asincrona de autorización: {str(e)}")
    ...
    def upload_thread():
        try:
            ...
            if drive_url:
                ...
                logger.info(f"✅ PDF de revista subido: {drive_url} (Revista ID: {revista_id})")
            else:
    ...
    def upload_thread():
        try:
            ...
            if drive_url:
                ...
                logger.info(f"✅ Imagen de revista subida: {drive_url} (Revista ID: {revista_id})")
            else:
...
def eliminar_pdf_de_drive(file_id):
    ...
    if not file_id:
        logger.warning("⚠️ No se proporciono file_id para eliminar")
        return False
